...
| Fase | Estado | Dónde |
|---|---|---|
| F0 · store y ruta anidada | ✅ | `core/messaging/chat.store.ts`, `app.routes.ts` (`RUTAS_ANIDADAS`), `.pantalla-a-sangre` en `redsat.css` |
...
- **D9 · Tema oscuro obligatorio.** Los colores del chat se definen como tokens propios
  ...
  `--chat-no-leido`) en `redsat.css`, con los tres bloques de siempre (`:root`,
...
- `styles/redsat.css` — tokens `--chat-*` (D9)
