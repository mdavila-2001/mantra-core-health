 ...
 * el nombre del escenario y redirige al destino. Así la elección viaja en **una
 * sola navegación** y no hay una ventana en la que la aplicación pida datos
 * antes de que el arnés sepa qué tiene que responder.
 *
 * Cypress limpia las cookies entre pruebas, así que cada prueba vuelve a
 * declarar el suyo y ninguna hereda el de la anterior.
 *
 * ## Dónde corre este archivo
 *
 * En los **dos** lados: el arnés de Node lo lee para resolver qué responder, y
 * las pruebas lo importan para nombrar el escenario que piden. Por eso no tiene
 * ninguna dependencia — ni de Node ni de Cypress — y es solo datos y tipos.
 */
...
export interface Escenario {
  /** Para qué existe. Aparece en el README y en el inventario generado. */
   ...
   * siguen esperando por condiciones —las de Cypress, que reintentan— nunca por
   * tiempo.
   */
