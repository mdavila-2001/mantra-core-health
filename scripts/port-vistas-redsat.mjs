/* ============================================================================
    ...
    pantallas maquetadas contra REDSAT v1.1. Cada archivo es una página
    ...
    Acá se separa lo que se repite de lo que no:
      · el marco sale UNA vez, a los dos shells de src/app/features/redsat/shell
    ...
    correr, lo pisa. Ver PORT-REDSAT.md.
    ========================================================================== */
...
const SALIDA = join(FRONT, 'src/app/features/redsat');
...
function cablear(documento, archivoOrigen, avisos) {
  ...
  documento.querySelectorAll('a[href]').forEach((a) => {
    ...
    /* `?estado=paso2` es la navegación entre etapas de un formulario: la misma
       ...
       y lo atiende RedsatRuntimeService.fijarEstado(). */
  ...
  documento.querySelectorAll('img[src], use[href]').forEach((el) => {
    ...
    if (valor && valor.includes('imagenes/')) {
      el.setAttribute(attr, `/redsat/imagenes/${basename(valor)}`);
    }
  });
  ...
  documento.querySelectorAll('[style*="imagenes/"]').forEach((el) => {
    el.setAttribute(
      ...
      el
        .getAttribute('style')
        .replace(/url\(['"]?[^'")]*imagenes\/([^'")]+)['"]?\)/g, "url('/redsat/imagenes/$1')"),
    );
  });

  /* El rol y el estado del interruptor de tema los pone ahora
     RedsatThemeToggleDirective, que es quien sabe qué tema se está pintando.
...
for (const archivo of ['vistas.manifest.json', 'marcos.crudos.json', 'redsat.routes.ts', 'redsat-nav.data.ts']) {
...
for (const vista of vistas) {
  ...
  const selector = `app-redsat-${sinTildes(baseNombre)}`;
  ...
  if (/app-theme-toggle/.test(plantilla)) {
    usados.push({
      simbolo: 'RedsatThemeToggleDirective',
      desde: '@core/redsat/redsat-theme-toggle.directive',
    });
  }
...
`,
  ...
  generadas.push({
    ...
    importacion: `@features/redsat/${vista.modulo.segmento}/${vista.slug}/${vista.slug}`,
  });
...
const navGenerado = `/* ============================================================================
    ...
    ARCHIVO GENERADO por scripts/port-vistas-redsat.mjs. Sale del propio nav de
...
export interface SeccionRedsat {
  readonly etiqueta: string;
  readonly ruta: string;
}

/** Un módulo del nav, con sus secciones maquetadas. */
export interface ModuloRedsat {
  readonly segmento: string;
  readonly rotulo: string;
  readonly secciones: readonly SeccionRedsat[];
}

export const MODULOS_REDSAT: readonly ModuloRedsat[] = ${JSON.stringify(
  [...navPorModulo.values()],
  null,
  2,
...
export const MODULO_POR_SEGMENTO = new Map<string, ModuloRedsat>(
  MODULOS_REDSAT.map((modulo) => [modulo.segmento, modulo]),
);
`;
writeFileSync(join(SALIDA, 'redsat-nav.data.ts'), navGenerado, 'utf8');
...
const grupoConMarco = (segmento, lista, marco) => `  {
    path: '${segmento}',
    loadComponent: () =>
      import('@features/redsat/shell/${marco.archivo}').then((m) => m.${marco.clase}),
    children: [
...
const bloquesPrivados = [...porModuloRutas.entries()]
  ...
  .map(([segmento, lista]) =>
    grupoConMarco(segmento, lista, { archivo: 'redsat-shell', clase: 'RedsatShell' }),
  )
  .join('\n');

const bloquePublico = publicas.length
  ? grupoConMarco(publicas[0].modulo.segmento, publicas, {
      archivo: 'redsat-public-shell',
      clase: 'RedsatPublicShell',
    })
  : '';
...
`;
writeFileSync(join(SALIDA, 'redsat.routes.ts'), rutasGeneradas, 'utf8');
