...
La **lámina 340**, titulada **«Uréteres»**, pertenece a **Abdomen > Riñones y glándulas suprarrenales**. Dentro del Atlas, debe entenderse como una unidad visual de anatomía macroscópica: su significado principal no está en una definición textual aislada sino en las relaciones espaciales que la ilustración permite observar.
...
- **Atla — vaginal** — `organo_viscera` — `consensus_high`
- **Wolff) — arteria** — `arteria` — `consensus_high`
- **Duodeno** — `organo_viscera` — `consensus_high`
- **Ligamento(s) — uréter femenino** — `ligamento` — `consensus_high`
- **Nervio(s) — obturador** — `nervio` — `consensus_high`
- **Vasos — ováricos** — `vaso` — `consensus_high`
- **Vasos — testiculares** — `vaso` — `consensus_high`
- **Vejiga urinaria** — `organo_viscera` — `consensus_high`
- **Vejiga urinaria — mucosa** — `organo_viscera` — `consensus_high`

## Evidencia OCR del encabezado

```text
Uréteres 4
Riñones y glándulas suprarrenales o Lámina 340
```

## Relaciones seguras

- `PLATE_340 -> PERTENECE_A_SUBREGION -> R04S08`
- `PLATE_340 -> PERTENECE_A_REGION -> R04`
