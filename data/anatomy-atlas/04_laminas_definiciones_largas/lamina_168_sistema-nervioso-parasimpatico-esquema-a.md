...
- **Nervio(s) — ramos** — `nervio` — `consensus_high`
- **Nervio(s) — ramo** — `nervio` — `consensus_high`
- **Ojo** — `ojo` — `consensus_high`
- **Plexo — pulmonar** — `plexo` — `consensus_high`
- **Próstata — inervación** — `organo_viscera` — `consensus_high`
- **Pulmones** — `organo_viscera` — `consensus_high`
- **Recto — inervación** — `organo_viscera` — `consensus_high`
- **Tráquea** — `estructura_anatomica_no_tipificada` — `consensus_high`
- **Vejiga urinaria — inervación** — `organo_viscera` — `consensus_high`

## Evidencia OCR del encabezado

```text
Sistema nervioso parasimpático: esquema A
Médula espinal o Lámina 168 —
```

## Relaciones seguras

- `PLATE_168 -> PERTENECE_A_SUBREGION -> R02S03`
- `PLATE_168 -> PERTENECE_A_REGION -> R02`
