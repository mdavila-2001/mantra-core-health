...
export class Thread {
  ...
  /**
   * Cierra el diálogo de reenviar sólo si el clic fue en el fondo.
   *
   * Antes la caja llevaba un `(click)` con `stopPropagation`, y eso deja un
   * manejador de clic en un elemento que el teclado no puede alcanzar —lo que
   * marca `click-events-have-key-events`—. Preguntando por el destino del
   * evento el comportamiento es el mismo y no hace falta el manejador de más.
   */
  protected alFondoDelReenvio(evento: Event): void {
