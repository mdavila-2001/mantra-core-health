...
| --- | --- | --- |
| `Vistas/HTML/` | Maquetas HTML estáticas contra **ALOVIDA v1.1** | 7 módulos, 133 archivos |
...
`src/styles/alovida.css` es **generado**: sale de `_assets/alovida.css` en la
bóveda con `node scripts/sync-alovida.mjs`. Nunca se edita a mano — el próximo
sync lo pisa. El script hace dos cosas y ninguna más:

- las tipografías pasan a servirse desde `/alovida/tipografias/` (`public/`);
...
`data-tema` lleva siempre el tema **ya resuelto**, porque la hoja de ALOVIDA no
...
`AlovidaRuntimeService` (`core/alovida/alovida-runtime.service.ts`) es el puerto de
`_assets/alovida.js`.
...
- el conmutador de tema, que gobierna `AlovidaThemeToggleDirective` contra
...
`node scripts/port-vistas-alovida.mjs` genera `src/app/features/alovida/`. De cada
...
| `/buscar/*` | V65 buscador | `AlovidaPublicShell` |
| `/datos-compartidos/*` | V02 common | `AlovidaShell` |
| `/terminologia/*` | V03 terminology | `AlovidaShell` |
| `/directorio/*` | V04 directory | `AlovidaShell` |
| `/personas/*` | V05 profiles | `AlovidaShell` |
| `/accesos/*` | V06 authz | `AlovidaShell` |
...
marco ALOVIDA directamente. No es un cambio de CSS: ALOVIDA compone al revés —el
...
```bash
node scripts/sync-alovida.mjs        # la hoja normativa
node scripts/port-vistas-alovida.mjs # las 126 pantallas y sus rutas
```
