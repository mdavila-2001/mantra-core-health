...
- Peldaño: `TESTED` para la generación reproducible, la pantalla de Cotizaciones, la navegación y la lectura de órdenes diagnósticas propias.
...
| H2.S1.M2 (parcial) | Órdenes diagnósticas propias se cargan desde `/diagnostic-results/me/orders`; sus conceptos se traducen antes de pintarse. | Cotizaciones 3/3; no se imprimen IDs de orden ni de concepto |

## A medias

### H2.S1.M2 — Fuentes y documentos reales
- Qué anda: se muestran referencias de maqueta con procedencia y precios no publicados sin valor de reemplazo; las órdenes diagnósticas del titular se consultan en el endpoint propio y aparecen con etiqueta de terminología.
- Qué no anda: las recetas y servicios médicos no se componen todavía en esta pantalla.
...
- `yarn test --watch=false --include=...cotizaciones...` → 5/5 PASS antes de incorporar documentos; `...cotizaciones.spec.ts` → 3/3 PASS después de incorporar órdenes propias.
