...
| **Quien sea dueño del CI** | **El gate `verificar` está rojo para todo el mundo, y por una línea que falta.** `.github/workflows/ci.yml` corre `yarn env:generate` (línea 85) pero **nunca `yarn stock:generate`**, y `src/app/features/component-stock/component-index.generated.ts` está en `.gitignore` (línea 160). El paso «Tipos» muere siempre con `TS2307: Cannot find module './component-index.generated'` más seis `TS7006` en cascada. **Comprobado en cuatro PRs distintos y ajenos —#561, #564, #566 y el mío #565—: los cuatro con `verificar` y `e2e` en rojo y `dependencias` en verde.** Los tres primeros se mergearon igual. Mientras siga así, **el CI no es una barrera de nada**: es exactamente cómo D-08 llegó a `mockup` pasando por todos los gates. El arreglo es un `- run: yarn stock:generate` antes del paso «Tipos». **No lo toqué**: `.github/**` es infraestructura compartida y no es de esta línea |
