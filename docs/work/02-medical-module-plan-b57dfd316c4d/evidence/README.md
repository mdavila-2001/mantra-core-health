...
[`h1-required-ci/REPORT.md`](h1-required-ci/REPORT.md) documenta la validación API obligatoria de CI/departamento con pruebas unitarias, sin conexión de integración a una base no aislada. [`h1-three-given-names/REPORT.md`](h1-three-given-names/REPORT.md) registra que el alta presenta tres campos, pero los nombres desde el segundo se pliegan en `middleName`; falta almacenamiento separado y el intento dirigido tuvo una falla por ENOSPC.
