...
> **Estado al 2026-09-24:** F0.M1–M3, M5 y F2 ejecutados; F1 parcial; F3 tiene una corrección visual CORR-08 publicada en PR borrador #612, sin merge; F4 parcial (alta API con diez credenciales/adjuntos, PATCH propio, lock concurrente de agenda; smoke de navegador→API real con dos títulos, lectura, descarga propia, PATCH y relectura tras recargar; faltan varios posgrados/especialidades en ese actor); F5 parcial (typecheck, pruebas dirigidas, suite frontend completa y evidencia visual pasan; lint global, route-health y verificador Fable siguen bloqueados); F6 documentado en ramas aisladas. Ninguno de los 98 criterios incluidos alcanza el DoD end-to-end.
...
- Evidencia FE: 97 pruebas de componente, Playwright 4/4 con API simulada, typecheck, ESLint dirigido y build exit 0; foto móvil revisada en el worktree FE. El smoke Playwright `real-api` pasó 4/4: dos PDFs recibieron 201, alta 201, lectura propia 200, descargas 200 con bytes PDF, PATCH 204 y lectura 200 tras recargar. SQL verificó dos filas y dos archivos distintos para el perfil probado. Falta recorrer varios posgrados/especialidades con ese mismo actor; la matriz integral permanece en 0/98 HECHO. Evidencia en `evidence/h1-real-api/`.
- El smoke usó una especificación temporal basada en el Playwright existente y se retiró después. El primer intento ampliado encontró Angular apagado en 4390 (`ERR_CONNECTION_REFUSED`); tras iniciar `ng serve --configuration real-api`, el archivo completo pasó 4/4. API y frontend estuvieron aislados en puertos locales, CORS no cambió, `ORM_SCHEMA_SYNC=off` y los archivos temporales se almacenaron bajo `/tmp`. La prueba API PostgreSQL de diez credenciales sigue separada y no es el mismo actor del navegador. Ver [reporte del smoke](evidence/h1-real-api/REPORT.md).
...
- Reanudación del 2026-09-24: el propietario pidió continuar hasta completar y publicar lo ejecutable. La inspección de solo lectura observó PostgreSQL temporal en 55439/55440, Angular en 4300/4302/4387 y un runtime Node SSR (`dist/mantra-core-health/server/server.mjs`), con dueños y datos desconocidos. No se conectó ni se alteró proceso alguno; no se corrió otro journey. La prueba de dos títulos por actor sigue siendo parcial, la matriz no cambia y no se modificó código de producto en esta reanudación.
