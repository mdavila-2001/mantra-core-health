...
- Peldaño máximo: `VERIFIED` para persistencia/lectura segura de credenciales y para excluir reservas concurrentes entre consultorios del mismo profesional; también pasó un alta real desde navegador con ocho credenciales (dos universitarias, dos diplomados, dos maestrías y dos doctorados), tres especialidades, ocho PDFs distintos, ficha sin `fileId`, PATCH y relectura tras recargar. No cubre el editor completo ni todos los escenarios de H1–H8. El plan Médico integral sigue en 0/98 criterios `HECHO` y no alcanza `REGRESSION_VERIFIED`.
...
| Alta médica en navegador contra API real | Chromium con configuración `real-api`, API real por proxy de mismo origen y PostgreSQL 18 desechable; fotos móviles inspeccionadas en `evidence/h1-real-api/extended-smoke/` | Smoke previo 4/4 con dos títulos universitarios; continuación extendida 1/1: ocho cargas 201, alta 201, login y resumen propio 200, ocho descargas del titular 200 con firma `%PDF-1.4`, tres especialidades, ficha autenticada sin `fileId`, PATCH 204 y resumen tras recargar 200. Tras recargar permanecen las ocho filas, ocho archivos únicos y tres especialidades; SQL confirma dos filas/archivos por cada uno de cuatro tipos | No verifica el editor desde navegador/API real ni credenciales de tipo especialidad en el formulario; la integración API separada prueba el rechazo 403 a otro actor y diez tipos/filas. MED-E01 sigue parcial; no cambia el conteo 0/98 |
...
Una prueba de registro profesional pasó 8/8 en DB desechable después de cargar terminología sintética parcial. La integración PostgreSQL del alta de credenciales pasó 1/1: guardó diez entradas (dos títulos universitarios y dos de cada tipo de posgrado, incluida especialidad), sus tipos, instituciones y PDFs distintos; el resumen privado las relee, el de ficha omite `fileId`, sólo el dueño descarga y el reuso se rechaza sin cuenta parcial. El PATCH propio pasa 4/4 e incluye lectura posterior, controles de propiedad, mass assignment y lock real. Un Playwright compuesto previo con mocks apagados verificó dos títulos; la continuación extendida pasó 1/1 en un entorno PostgreSQL propio: en el mismo actor creó ocho credenciales del formulario (dos por tipo universitario, diplomado, maestría y doctorado), seleccionó tres especialidades, descargó los ocho PDFs, consultó la ficha sin `fileId`, editó una credencial y, tras recargar, volvió a verificar las ocho filas, ocho archivos únicos, tres especialidades y los datos editados. SQL corroboró los ocho registros, cuatro tipos y tres especialidades. Sigue faltando el editor desde interfaz contra API real, la credencial de tipo especialidad en el navegador y H1/H2–H8 integrales; MED-E01 continúa A MEDIAS y la matriz en 0/98.
