...
La lectura de `register-practitioner.html` encontró tres campos visibles (Primer, Segundo y Tercer nombre), además de campos dinámicos para nombres posteriores. La prueba de alta comprueba que el segundo, tercero y los adicionales se envían unidos en `middleName`; API los persiste en la única columna de segundo nombre y la lectura propia devuelve sólo `middleName`. No hay propiedad `thirdName` ni columna persistente separada en la entidad actual. El spec FE dirigido produjo 97 aprobadas y una falla `ENOSPC` en el test de tipos de credencial; las pruebas específicas de tercer nombre están incluidas en las aprobadas. L0168 continúa `A MEDIAS / TESTED`; no se cambia DDL/modelo y no se infiere una reversión confiable del nombre unido.
