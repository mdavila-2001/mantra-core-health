...
- **R-008 — documentos de credenciales:** se usa el campo `fileId` y la autorización existentes; el resumen autenticado propio devuelve el identificador del PDF, el resumen público lo omite y la descarga autoriza al dueño. PostgreSQL efímero comprobó diez registros con dos credenciales de cada tipo, instituciones y archivos separados. No se cambió el catálogo de títulos ni se considera prueba del journey FE→API real.
