 ...
 *
 * Responde los seis endpoints que la aplicación usa en los flujos que esta
 * suite cubre, con el **mismo contrato** que la API real: el sobre de error de
 * `docs/api/error-model.md` (`code`, `message`, `timestamp`, `path`) y el par
 * de tokens de `POST /iam/auth/login`.
 *
 * Qué responde en cada caso lo decide el escenario que la prueba dejó en una
 * cookie. La cookie es por perfil de navegador y cada prueba levanta el suyo,
 * así que dos pruebas simultáneas pueden pedir respuestas opuestas al mismo
 * proceso sin interferirse.
 */
